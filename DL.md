...
**3. 计算交叉熵损失**
 ...
 $$H[y, \hat y] = \sum_{j} -\ y_j \log \hat y_j$$

交叉熵损失等价于负对数似然，后续计算梯度就相当于变相进行了最大似然估计

补充：1. 为什么交叉熵损失等价于负对数似然；2. 交叉熵损失是什么 3. 为什么不直接进行最大似然估计



**4. 对损失计算梯度**

相当于降低 预测是某种类别后结果真的是该类别的惊异 即 随着梯度降低，这种预测会越来越准，最后达到有较高预测准确率的效果
